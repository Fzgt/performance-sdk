...
var ReportData = /*#__PURE__*/function () {
  ...
  _createClass(ReportData, [{
    key: "sendToAnalytics",
    value: function sendToAnalytics(level, body, uri) {
      var logurl = this.logUrl; //临时更换其他url
      ...
      if (level == _types.AskPriority.URGENT) {
        if (!!_constants.W.fetch) {
          ...
          xhr.open('post', logurl, true); // 设置请求头
...
var getNetworkInformation = function getNetworkInformation() {
  ...
  } else {//todo 这里我们使用多普勒测速法或者直接用图片探测法
  }
...
var getNavigationTiming = function getNavigationTiming() {
  ...
  var n = _constants.WP.getEntriesByType('navigation')[0]; // In Safari version 11.2 Navigation Timing isn't supported yet
  ...
  var responseEnd = n.responseEnd; // We cache the navigation time for future times
 ...
 */
var initLayoutShift = function initLayoutShift(performanceEntries) {
  var lastEntry = performanceEntries.pop(); // Only count layout shifts without recent user input.
...
var initFirstInputDelay = function initFirstInputDelay(performanceEntries) {
  ...
  (0, _performanceObserver.poDisconnect)(1); //初始化lcp
  ...
  (0, _log.logMetric)(_metrics.tbt.value, 'tbt'); // TBT with 5 second delay after FID
  ...
  setTimeout(function () {
    (0, _log.logMetric)(_metrics.tbt.value, "tbt10S"); //FID被激活以后10S的整体数据消耗
...
var initPerformanceObserver = function initPerformanceObserver() {
  ...
  _observeInstances.perfObservers[2] = (0, _performanceObserver.po)('largest-contentful-paint', _paint.initLargestContentfulPaint); //收集页面全部资源性能数据
...
var ErrorTrace = /*#__PURE__*/function () {
  ...
  _createClass(ErrorTrace, [{
    ...
    key: "run",
    value: function run() {
      this.networkError(); //触发全体数据监听错误

      this.globalError(); //触发promise的错误
 ...
 */
var reportStorageEstimate = function reportStorageEstimate(storageInfo) {
...
var Yideng = function Yideng() {
  ...
  _config.config.reportData = insReportData; //对外暴露上传接口

  this.reportData = insReportData; //集合数据汇总
  ...
  (0, _log.logData)('navigationTiming', (0, _getNavigationTiming.getNavigationTiming)()); //记录用户的网速 H5+多普勒测速

  (0, _log.logData)('networkInformation', (0, _getNetworkInformation.getNetworkInformation)()); //管理离线缓存数据
...
},{}]},{},["../node_modules/parcel-bundler/src/builtins/hmr-runtime.js","performance/index.ts"], null)
